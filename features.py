...
https://gitlab.irit.fr/melodi/andiamo/relation-classification-features/-/blob/main/features.py
which is to be based on https://github.com/gucorpling/DisCoDisCo/blob/master/gucorpling_models/rel/features.py

The papers are
Andiamo: https://aclanthology.org/2024.codi-1.9/
DisCoDisCo: https://aclanthology.org/2021.disrpt-1.6/
...
headers = ['doc', 'unit1_toks', 'unit2_toks', 'unit1_txt', 'unit2_txt', 'u1_raw', 'u2_raw', 's1_toks', 's2_toks', 'unit1_sent', 'unit2_sent', 'dir', 'rel_type', 'orig_label', 'label']
...
news_corpora = ["eng.rst.rstdt","eng.pdtb.pdtb","deu.rst.pcc","por.rst.cstn","zho.pdtb.cdtb"]
chat_corpora = ["eng.rst.stac", "eng.sdrt.stac", "eng.sdrt.msdc"]
other_corpora = ["eng.dep.scidtb", "ces.rst.crdt", "ita.pdtb.luna", "eng.rst.oll"]
gum_corpora = ["eng.rst.gum", "eng.pdtb.gentle", "eng.erst.gum", "eng.pdtb.gum", "eng.erst.gentle"]
science_corpora = ["eng.dep.scidtb", "zho.dep.scidtb"]

corpora_with_doc_genre = gum_corpora + ["rus.rst.rrt", "eus.rst.ert", "fra.sdrt.annodis", "nld.rst.nldt",
                          "spa.rst.rststb", "spa.rst.sctb", "spa.rst.sctb", "fas.rst.prstc",
                          "eng.erst.gum", "zho.rst.gcdt"]

genre_documented_corpora = news_corpora + science_corpora + chat_corpora + other_corpora + corpora_with_doc_genre
undocumented_corpora = [
    "tha.pdtb.tdtb", "eng.rst.sts", "eng.pdtb.tedm", "por.pdtb.crpc", "tur.pdtb.tdb", "eng.dep.covdtb", "eng.rst.oll", "zho.rst.sctb", "por.pdtb.tedm", "tur.pdtb.tedm"
]

"""
{'eng.dep.scidtb', 'ces.rst.crdt', 'deu.rst.rstdt-translated', 'tha.pdtb.tdtb', 'eng.rst.sts', 'zho.dep.scidtb', 'zho.rst.sctb', 'eng.pdtb.tedm', 'zho.rst.gcdt', 'por.pdtb.crpc', 'tur.pdtb.tdb', 'eng.sdrt.msdc', 'eng.pdtb.gum', 'por.pdtb.tedm', 'tur.pdtb.tedm', 'ces.rst.rstdt-translated', 'eng.rst.oll', 'stats.csv.xlsx', '~$stats.csv.xlsx', 'eng.dep.covdtb'}
"""
...
def get_genre(corpus, doc):
    ...
    if corpus in science_corpora:
        return "science"

    if corpus == "eng.rst.oll":
        return "discussion"

    if corpus == "ita.pdtb.luna":
        return "conversation"

    if corpus == "eng.dep.covdtb":
        return "academic"

    if corpus == "ces.rst.crdt":
        # journalisitic corpus
        return "news"

    if corpus in chat_corpora:
        return "chat"
    ...
    if corpus in gum_corpora or corpus == "zho.rst.gcdt":
...
#             head = 0 if fields[6] == "0" else int(fields[6]) + offset
    ...
    if corpus not in genre_documented_corpora:
        logger.warning(f"Providing empty genre for {corpus} as it is not documented.")
    genres_for_docs =  {doc: get_genre(corpus, doc) for doc in tokmap.keys()}
    output = []
    ...
    for i, line in enumerate(lines):
        if "\t" in line and i > 0:  # Skip header
            ...
            # Use the cached genre for dooc instead of the logic every everytime
            feats["genre"] = genres_for_docs[feats["doc"]]
...
def prepare_output(rows, headers):
    output = []
    ordered = []
    for row in rows:
...
def save_output(output, save_dir, corpus, filename):
    if not os.path.exists(os.path.join(save_dir, corpus)):
        os.makedirs(os.path.join(save_dir, corpus))

    with io.open(os.path.join(save_dir, corpus, filename),
                 'w', encoding="utf8", newline="\n") as f:
        f.write("\n".join(output) + "\n")

def process_corpus(corpus_root, save_dir):
    # Could be handled with pathlib.Path
    files = glob(corpus_root + "*.rels")

    for file_ in files:
        rows = process_relfile(file_, file_.replace(".rels",".conllu"), corpus, keep_all_columns=True)
        output = prepare_output(rows, headers)
        filename = file_.split(os.sep)[-1]
        save_output(output, save_dir, corpus, filename)

        #print ("\n".join(output[:100]))
        #quit()
        # make folder for data with the same structure as DISRPT 2023


if __name__ == "__main__":
    p = ArgumentParser()
    p.add_argument("--disrpt_data", action="store", default="../disrpt2023/data/dataset",
                   help="Directory with DISRPT repo data folder")
    p.add_argument("-c","--corpus", default="eng.rst.gum", help="corpus name")
    p.add_argument("--save_dir", default="data", help="Folder to save the new .rels files.")

    opts = p.parse_args()

    # Can be handled with pathlib.Path
    ...
    # For a corpus directory
    process_corpus(corpus_root, save_dir)
